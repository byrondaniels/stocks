/**
 * Spinoff Investment Analyzer
 * Comprehensive 4-phase spinoff analysis using Gemini AI
 *
...
import { getGeminiClient, GEMINI_MODELS, DEFAULT_GENERATION_CONFIG, parseGeminiJSON } from './ai/index.js';
import { buildSpinoffAnalysisPrompt } from '../prompts/index.js';
...
export async function analyzeSpinoffWithGemini(
  ...
  try {
    ...
    const analysis = parseGeminiJSON<SpinoffAnalysis>(text, fallback);

    // Validate structure
    if (!analysis.executive_summary || !analysis.phase1_screening ||
        !analysis.phase2_quality || !analysis.phase3_valuation) {
      console.warn('[Gemini] Invalid spinoff analysis structure, using fallback');
      return fallback;
    }
