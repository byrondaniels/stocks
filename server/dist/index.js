...
import { fetchAndStoreHistoricalPrices, calculateMovingAverage, getPriceHistorySummary, getLatestStoredPrice, calculate50DMA, } from "./services/priceHistory.js";
 ...
 */
app.post("/api/portfolio/:ticker/refresh", async (req, res) => {
...
app.get("/api/portfolio", async (_req, res) => {
    try {
        ...
        const portfolioWithMetrics = await Promise.all(portfolioItems.map(async (item) => {
            try {
...
app.put("/api/portfolio/:ticker", async (req, res) => {
    ...
    try {
        const updatedItem = await Portfolio.findOneAndUpdate({ ticker }, { $set: updateFields }, { new: true, runValidators: true });
        if (!updatedItem) {
            res.status(404).json({
                error: `Stock ${ticker} not found in portfolio.`,
            });
...
async function writeStoredInsider(ticker, record) {
    try {
        await InsiderTransaction.updateOne({ ticker }, {
            ticker: record.ticker,
            cik: record.cik,
            transactions: record.transactions,
            summary: record.summary,
